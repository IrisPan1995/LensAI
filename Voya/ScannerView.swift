...
struct ScannerView: View {
    ...
    @State private var analysisStatus = "Detecting content..."
        ...
        GeometryReader { geo in
            ...
            ZStack {
                ...
                HStack(spacing: 40) {
                    ...
                    // Shutter button
                    Button {
                        // Set crop rect (normalized 0…1) so only the scan box area is captured
                        camera.previewSize = geo.size
                        camera.cropRect = CGRect(
                            x: boxOrigin.x / geo.size.width,
                            y: boxOrigin.y / geo.size.height,
                            width: boxSize / geo.size.width,
                            height: boxSize / geo.size.height
                        )
                        camera.takePhoto()
                    } label: {
                        ZStack {
    ...
    private func startAnalysis(image: UIImage) {
        ...
        analysisProgress = 0
        errorMessage = nil
        analysisStatus = "Detecting content..."
